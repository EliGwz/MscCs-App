...
public class FragmentCourseFinancial extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view =  inflater.inflate(R.layout.fragment_course_financial,container,false);
        String stringUrl = "https://www.msc-cs.hku.hk/Curriculum/Courses";
        courseListView = view.findViewById(R.id.financialCourseListView);
    ...
    private class DownloadWebpageTask extends AsyncTask<String, Void, Document> {
        ...
        protected void onPostExecute(Document doc){
            ...
            map.put("course_no", eachLineElements.get(7).select("td").select("p").get(1).text());
            map.put("course_name", eachLineElements.get(7).select("td").select("p").get(2).text());
            list.add(map);
